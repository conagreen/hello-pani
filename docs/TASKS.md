...
이 문서는 이 프로젝트의 구현 단위를 기록한다.
각 단위는 작고 검증 가능하게 나뉘어 있어, 한 단위씩 구현하면서 [DECISIONS.md](DECISIONS.md)와 [DOMAIN.md](DOMAIN.md)의 설계 의도가 코드까지 일관되게 이어지도록 했다.
...
- [x] Actuator / Micrometer 메트릭 노출
- [x] 핵심 메트릭 추가: Redis gate 성공 / 실패, DB 재고 선점 성공 / 실패, Booking 성공 수, 결제 실패 유형, 503 비율
- [x] k6 정합성 시나리오 작성
- [x] k6 멱등성 시나리오 작성
- [ ] 선택 시나리오 작성: 스파이크, Redis 장애
- [x] 부하 테스트용 compose 또는 실행 스크립트 추가
- [x] 부하 테스트 실행 명령 문서화

완료 조건:

- [x] 다수 동시 요청에서 Booking `CONFIRMED`가 정확히 10건인지 확인
- [x] 같은 checkoutId 동시 요청에서 결제 1건만 발생하는지 확인
- [x] Redis 장애 시 `503`이 발생하고 DB 우회가 없는지 확인
- [x] 테스트 결과를 README에 요약할 수 있는 형태로 저장
