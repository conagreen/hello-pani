...
위와 같이 review 스크립트를 실행하면 아래와 같은 콘솔이 제공된다.

![img1](assets/1-running-review-sh.png)

스크립트를 실행하기 전에 `g`를 눌러 그라파나 환경도 추가로 구동할 수 있다.  
그라파나에서 각 테스트가 애플리케이션, mysql, redis 등에 어떤 영향을 주는지 시각화되어 제공된다.  
예시로 스트레스 테스트 `s`를 선택하고 `20s`를 옵션으로 입력해보자.  

![img2](assets/2-select-stress-testing-for-20-seconds.png)

그리고 앞서 구동한 그라파나에서 지표를 살펴볼 수 있다.  

![img3](assets/3-grafana.png)

성공 시 모든 단계가 ✓로 표시되며, 실패하면 어디서 깨졌는지가 한눈에 보인다. 실패 안했으면 좋겠다.  
